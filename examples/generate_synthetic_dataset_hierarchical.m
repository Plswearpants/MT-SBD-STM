...
for i = 1:length(theta_cap_values)
    for j = 1:length(area_ratio_values)
        ...
        [A0, A0_noiseless] = generate_kernels(area_ratio_values(j), ...
            fixed_params, LDoS_sim, sliceidx);
...
%% Helper Functions
function X0 = generate_activation_maps(theta_cap, fixed_params)
    % Generate activation maps for given theta_cap value
    % Initialize output
    X0 = zeros([fixed_params.image_size fixed_params.num_kernels]);
    
    % Generate theta values for each kernel
    theta = 0.8*theta_cap + 0.2*theta_cap * rand(1, fixed_params.num_kernels);
    theta = theta .* fixed_params.relative_theta_cap;
...
function [A0, A0_noiseless] = generate_kernels(area_ratio, fixed_params, LDoS_sim, sliceidx)
    % Generate kernels with specified area ratio
    ...
    for k = 1:fixed_params.num_kernels
        ...
        kernel_length = max(kernel_length, 3); % Ensure minimum size
