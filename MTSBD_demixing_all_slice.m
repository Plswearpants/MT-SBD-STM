function [ Aout, Xout, bout, extras ] = MTSBD_demixing_all_slice( Y, k, params, dispfun, kernel_initialguess, Max_iteration, maxIT)
    ...
    for iter = 1:maxIT
        ...
        for n = 1:kernel_num
            ...
            X_current = Xiter(:,:,n);
            Yiter = Y_residual + (iter > 1) * (1-1/(faint_factor*iter+1))*convfft3(A{n}, X_current(:,:,ones(1,size(Y_sum,3)))) + (1/(faint_factor*iter+1))*Y_sum;
        ...
        % only compute the quality metrics for the first slice
