...
%   'InteractiveThreshold': (optional) Set to true to enable interactive threshold selection
%                          Default is false, which uses max as threshold
...
p = inputParser;
addParameter(p, 'InteractiveThreshold', false, @islogical);
addParameter(p, 'Direction', 'none', @(x) ismember(x, {'none', 'horizontal', 'vertical'}));
parse(p, varargin{:});
useInteractive = p.Results.InteractiveThreshold;
...
for k = 1:(points)
    ...
    % Get thresholds either interactively or using median
    if useInteractive
        [threshold_col, threshold_row] = getVarianceThresholds(col_var, row_var, x_num);
    else
