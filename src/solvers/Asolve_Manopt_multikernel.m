    ...
    %
        ...
        if nvarargin < idx || isempty(varargin{idx})
            if strcmp(Xsolve,'FISTA')
                xinit = Xsolve_FISTA_multikernel(Y, Ain, lambda, mu, [], xpos); % initialize activation map with FISTA
            elseif strcmp(Xsolve,'pdNCG')
                xinit = Xsolve_pdNCG(Y, Ain, lambda, mu, [], xpos); % initialize activation map with pdNCG
            end
        else
            xinit = varargin{idx}; % initialize activation map with given activation map
        end
        ...
        problem.M = spherefactory(prod(k)*num_kernels);
