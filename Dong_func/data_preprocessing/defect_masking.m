function [dIdV_masked, QPI_masked] = defect_masking(dIdV, index)
% Description: 
...
clear y
sigma = 10;
%imagesc(dIdV(:,:,index)) % show dIdV at specified voltage
...
QPI_masked(:,:,:) = abs(fftshift(fft2(dIdV_masked(:,:,:))));
end
